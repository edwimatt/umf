	...
	class AdminDashboardController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    */
        ...
        public function getIndex() {
            ...
            {
                $from_date = new Carbon($_GET['from_date']);
            }
            if(isset($_GET['to_date']) && $_GET['to_date'] != "")
            {
                $to_date = new Carbon($_GET['to_date']);
            }
            $data = array();
            ...
            try {
                $user = User::getTotalUser($data);
                $user_live = User::getTotalLiveUser($data);


                $data['users'] = $user[0]->users;
                $data['users_live'] = $user_live[0]->users_live;

                $data['from_date'] = $from_date;
                $data['to_date'] = $to_date;
                $this->cbView('admin_dashboard_reports',$data);
            } catch (\Exception $e) {
